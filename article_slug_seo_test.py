#!/usr/bin/env python3
"""
Comprehensive Article Slug System and SEO Optimization Testing
for The Crewkerne Gazette

This test suite focuses on:
1. SEO Routes Validation (robots.txt, sitemap.xml, news-sitemap.xml)
2. Article Slug System Testing
3. Meta Tags and Structured Data
4. Backend Route Validation
5. Emergency System Compatibility
"""
...
class ArticleSlugSEOTester:
    ...
    def authenticate(self):
        ...
        if status == 200 and isinstance(response, dict) and 'access_token' in response:
            ...
            return self.log_test("Admin Authentication", True, f"Token: {self.token[:20]}...")
        else:
    ...
    def test_seo_routes(self):
        ...
        if status == 200:
            try:
                ...
                news_urls = root.findall('.//news:news', news_namespace)
    ...
    def test_article_slug_generation(self):
        ...
        test_articles = [
            {
                "title": "Breaking News: Major Development in Crewkerne",
                "content": "This is a test article for slug generation testing. It contains enough content to meet the minimum requirements for article creation.",
                "category": "NEWS",
                "subheading": "Testing slug generation functionality",
                "is_breaking": True,
                "tags": ["test", "slug", "breaking"]
            },
            {
                "title": "Music Review: Local Band Rocks the Stage!",
                "content": "Another test article to verify slug uniqueness and generation. This article has different content and category to test the system thoroughly.",
                "category": "MUSIC", 
                "subheading": "Testing slug uniqueness",
                "is_breaking": False,
                "tags": ["music", "review", "local"]
            }
        ]
        ...
        for i, article_data in enumerate(test_articles):
            ...
            try:
                ...
                if status == 200:
                    ...
                    self.log_test(f"Article Creation #{i+1}", False, f"Status: {status}")
    ...
    def test_slug_based_routes(self):
        ...
        if not self.created_articles:
            self.log_test("Slug Route Testing", False, "No articles created for testing")
            return
    ...
    def test_emergency_system_compatibility(self):
        """Test that SEO routes work without database connection"""
        print("\n🆘 EMERGENCY SYSTEM COMPATIBILITY")
        print("-" * 40)
        
        # Test SEO routes (should work even if database is down)
        # These routes have fallback mechanisms
        
        # Test robots.txt (should always work)
        status, content = self.make_request('GET', 'robots.txt', expect_json=False)
        robots_emergency = status == 200 and 'User-agent: *' in content
        self.log_test("Robots.txt emergency compatibility", robots_emergency,
                     f"Status: {status}, Basic content present: {robots_emergency}")
        
        # Test sitemap.xml (should return basic sitemap on database failure)
        status, content = self.make_request('GET', 'sitemap.xml', expect_json=False)
        sitemap_emergency = status == 200 and 'crewkernegazette.co.uk' in content
        self.log_test("Sitemap.xml emergency fallback", sitemap_emergency,
                     f"Status: {status}, Contains domain URLs: {sitemap_emergency}")
        
        # Test news-sitemap.xml (should return empty but valid XML on failure)
        status, content = self.make_request('GET', 'news-sitemap.xml', expect_json=False)
        news_sitemap_emergency = status == 200 and 'urlset' in content
        self.log_test("News sitemap emergency fallback", news_sitemap_emergency,
                     f"Status: {status}, Valid XML structure: {news_sitemap_emergency}")
    ...
    def run_comprehensive_test(self):
        ...
        print("🚀 CREWKERNE GAZETTE - ARTICLE SLUG SYSTEM & SEO OPTIMIZATION TESTING")
        print("=" * 80)
...
def main():
    """Main test execution"""
    # Test with production URL from frontend .env
    tester = ArticleSlugSEOTester("https://crewkernegazette.co.uk")
