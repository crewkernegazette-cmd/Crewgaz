...
<body>
    ...
    {f"<h2>{article_obj.subheading}</h2>" if article_obj.subheading else ""}
    <p>{article_obj.content[:300]}...</p>
    <p><em>View the full article at: <a href="https://crewkernegazette.co.uk/article/{article_id}">https://crewkernegazette.co.uk/article/{article_id}</a></em></p>
</body>
</html>"""
        ...
        # For regular users, return a redirect to the frontend 
        # This will be handled by the hosting platform to serve React app
        raise HTTPException(
            status_code=302, 
            detail="Redirect to frontend",
            headers={"Location": f"https://crewkernegazette.co.uk/article/{article_id}"}
        )
